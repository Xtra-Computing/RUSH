...
def calculate_average_latency(directory_path, fraud_file_path, increment_file_path):
    ...
    with open(increment_file_path, 'r') as file:
        for i, line in enumerate(file):
            ...
            if source in caught_timestamps or target in caught_timestamps:
                money += weight 
                catch_count+= 1
            
            if source in fraud_nodes or target in fraud_nodes:
                total_count += 1
    ...
    summary_file_path = directory_path + '/ratio3.txt'
